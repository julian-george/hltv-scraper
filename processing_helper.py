...
def get_map_vector(map_name):
    map_dict = {f"map_{name.lower()}": map_name in name for name in map_list}
    return map_dict
...
def process_maps(
    ...
    for curr_map in maps_to_process:
        ...
        # i-629
        w["match_id"] = related_match["hltvId"]
        ...
        with frame_lock:
            # feature_data.matrix = np.vstack((feature_data.matrix, np.array(w)))
            feature_data.frame = pd.concat([feature_data.frame, pd.DataFrame([w])])
